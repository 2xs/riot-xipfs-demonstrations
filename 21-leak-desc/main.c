...
int main(int argc, const char *argv[]) {
    ...
    (void)printf("ok %s %d %x\n", argv[0], (int)10, 0xCAFEBABE);
    int res = snprintf(buffer, sizeof(buffer), "ok %s %d %x\n", argv[0], (int)11, 0xDEADBEEF);
    if (res < 0) {
        printf("snprintf failed.\n");
    } else {
        (void)printf(buffer);
    }
    res = snprintf(buffer, sizeof(buffer), "0123456789ABCDEF"
                                           "0123456789ABCDEF"
                                           "0123456789ABCDEF"
                                           "0123456789ABCDEF"
                                           "0123456789ABCDEF");
    if ((res < 0) || ((unsigned)res >= sizeof(buffer))) {
        printf("snprintf failed as expected.\n");
        res = 0;
    } else {
        printf("snprintf DID NOT fail as expected.\n");
